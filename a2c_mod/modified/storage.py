...
class EvoACStorage(object):
    def __init__(self, max_episode_steps, num_pop, obs_shape, is_cuda=True, value_coeff=0.5,
    ...
    def reset_buffers(self):
        """
        Creates and/or resets the buffers - each of size (max_episode_steps, num_pop) -
        storing: - rewards
                 - states
                 - actions
                 - log probabilities
        ...
        :return:
        """
        self.rewards = self._generate_buffer((self.max_episode_steps, self.num_pop))
        ...
        self.values = self._generate_buffer((self.max_episode_steps, self.num_pop))
        self.dones = self._generate_buffer((self.max_episode_steps, self.num_pop))
    ...
    def insert(self, step, pop_idx, reward, obs, action, log_prob, value, done):
        """
        ...
        :return:
        """
        self.rewards[step][pop_idx] = reward
        self.states[step + 1][pop_idx].copy_(self.obs2tensor(obs))
        self.actions[step][pop_idx].copy_(action)
        self.log_probs[step][pop_idx].copy_(log_prob)
        self.values[step][pop_idx] = value
        self.dones[step][pop_idx] = done
    ...
    def _discount_rewards(self, final_value, discount=0.99):
        ...
        r_discounted = self._generate_buffer((self.max_episode_steps, self.num_pop))
        ...
        R = self._generate_buffer(self.num_pop).masked_scatter((1 - self.dones[-1]).byte(), final_value)

        for i in reversed(range(self.max_episode_steps)):
            ...
            # todo: add GAE
            R = self._generate_buffer(self.num_pop).masked_scatter((1 - self.dones[-1]).byte(),
